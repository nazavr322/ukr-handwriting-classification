...
7. **Train and evaluate** - fine-tunes model pre-trained on MNIST on a training data from stage(6) and evaluates its performance on test data also from stage(6).
...
All the hyperparameters where fine-tuned with [`optuna`](https://github.com/optuna/optuna) framework, you can check out this code at `notebooks/optuna.ipynb`. In this case `SGD` optimizer with momentum performed better than `Adam`, with `ReduceOnPlateau` lr annealing strategy.
...
We have a very lightweight model, trained for only 30 epochs. On my laptop GPU training lasts for a few minutes at best. It generalizes pretty good on both tasks simultaneously. On the first confusion matrix you can see that model sometimes confuses such pairs of ukrainian letters as `і` and `ї`, `е` and `є`.    
